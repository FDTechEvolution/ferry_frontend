...
<div class="col-12 py-3" style="background-color: {{ $bg }};">
    <div class="row">
        <div class="col-12 col-md-3 text-center check-in-col mb-3 mb-md-0">
            ...
            <img src="{{ asset('icons/check-in/'.$type.'.png') }}" class="d-none d-md-block w-75">
        ...
        <div class="col-12 col-md-9">
            ...
            <div class="flickity-preloader flickity-white"
                data-flickity='{ "autoPlay": false, "cellAlign": "left",
                ...
                @foreach ($stations as $index => $item)
                    ...
                    <div class="col-5 col-md-3 col-md-custom px-1">
                        <div class="cursor-pointer text-center check-in-list" onClick="updateMap(`{{ $item['map'] }}`, '{{ $type }}', {{ $index }})">
                            <p class="text-center mt-1 mb-0 --lh-16 fw-medium small s_name name-{{ $type }}-{{ $index }}" style="min-height: 38px;">{{ $item['name'] }}</p>
                            <img src="{{ asset('icons/check-in/'.$type.'.png') }}" class="w-75">
                            @if($item['type'] == 'island' || $item['type'] == 'pier')
                                <p class="text-center mt-1 mb-0 --lh-16 smaller" style="color: {{ $color }};">({{ $item['piername'] }})</p>
