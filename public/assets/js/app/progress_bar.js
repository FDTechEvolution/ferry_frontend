...
function setAddonPrice(price_current, charge, price, status, person) {
    ...
    if(status === 'Y') {
        ...
        let set_price_to = price_current.value == '0' ? '0' : `${numberFormat(price_current.value * 1)} x ${person} = ${price_current.value * person} <span class="small">THB</span>`
        charge.innerHTML = `${set_price_to}`
